...
export function MentorSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  ...
  const displayName: string = storedUser.name ?? storedUser.fullName ?? "Mentor"
      ...
      <SidebarHeader className="border-b border-sidebar-border pb-4">
        <div className="flex items-center gap-3 px-2 py-1 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:px-0">
          ...
          <div className="flex flex-col group-data-[collapsible=icon]:hidden">
            <span className="text-sm font-bold leading-tight tracking-tight">Portal Mentor</span>
            <span className="text-xs text-muted-foreground">Bimbingan & Evaluasi</span>
          </div>
        </div>
      </SidebarHeader>
